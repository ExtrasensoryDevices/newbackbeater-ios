...
func CGImageSourceGIFFrameDuration(imageSource: CGImageSource, index: Int) -> NSTimeInterval {
    ...
    if let properties = imageSource.GIFPropertiesAtIndex(index),
        durationFromGif = durationFromGIFProperties(properties),
        capDuration = capDuration(durationFromGif) {
            return capDuration
    }
...
private func durationFromGIFProperties(properties: GIFProperties) -> Double? {
    ...
    if let concreteUnclampedDelayTime = unclampedDelayTime, concreteDelayTime = delayTime {
        return duration(concreteUnclampedDelayTime, concreteDelayTime)
    }
