...
class Animator: NSObject {
    ...
    /// If the gif goes back and forth
    let pingpong: Bool = true
    ...
    required init(data: NSData, delegate: Animatable) {
        ...
        prepareFrames(imageSource, size: delegate.frame.size)
    ...
    func updateCurrentFrame() {
        ...
        if timeSinceLastFrameChange >= frameDuration {
            ...
            if currentFrameIndex == 0 && increment == -1 {
                increment = 1
            } else if currentFrameIndex == animatedFrames.count - 1 {
                if pingpong {
                    increment = -1
                } else {
                    currentFrameIndex = -1
